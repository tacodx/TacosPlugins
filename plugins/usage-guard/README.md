...
The advisory ("soft") threshold set this way is always 15 points below the
...
**An unreadable config never *newly* enables enforcement — but it does not
always mean "safe" either.** If `config.json` (or a per-chat session file)
exists but cannot be parsed — corrupt JSON, a permissions problem, anything
other than "the file simply isn't there yet" — the guard never silently
falls back to the shipped enforce defaults on data it couldn't actually
read. But a mode a *readable* layer stated explicitly is preserved, not
discarded: if your `config.json` reads `"mode": "enforce"` and only your
per-chat session file happens to be corrupted, the guard is still
enforcing. Only when no readable layer named a mode at all does it degrade
to `dry-run`. Either way, both `/budget` and `explain` tell you the truth —
they print a line stating part of the config could not be read, together
with **the mode actually in effect**, and add "observing only, not
enforcing" only when that effective mode is really `dry-run`. Always read
that line rather than assuming; do not assume "config was unreadable" means
"nothing can be denied." A file that is genuinely missing (`ENOENT`) is not
this case at all — that's just "no config yet," and the normal defaults
apply with no warning printed.
...
- **The guard always fails open.** Missing credentials, an API error, a
  ...
  block you because it broke. The corollary: a broken guard silently
  protects nothing. If you care whether it's actually working, check
  `explain` — it tells you plainly when it's blind or degraded.
...
- **Tested against Claude Code 2.1.220 only.** The hook payload shape, the
  ...
  fan-out — `Agent` and `Workflow` — are all specific to that version and
  could change in a future Claude Code release. (The guard's matcher also
  lists `Task` defensively; `tool_name` for a subagent call is always the
  literal `"Agent"` in every version tested, so `Task` has never actually
  fired and is not something to rely on or plan around.)
