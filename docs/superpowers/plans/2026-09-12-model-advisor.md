...
- [ ] **Step 5: Mutation-check.** Make `switchingHelps` return `true` when the model is unknown; confirm a test fails. Make `binding` return the *lowest* percentage; confirm a test fails. Restore both.
